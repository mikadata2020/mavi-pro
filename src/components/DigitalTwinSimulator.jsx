        ...
        <div style={{ backgroundColor: '#111', padding: '20px', borderRadius: '12px', color: 'white' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                    <div style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>🏭 Digital Twin Live View</div>
                ...
                <div style={{ display: 'flex', gap: '20px', fontSize: '0.9rem' }}>
                    <div>⏱ Time: {(simState.stats.timeElapsed / 60).toFixed(0)}m</div>
                    <div>📦 Output: {simState.stats.output} units</div>
                    <div>⚡ Efficiency: {simState.stats.timeElapsed > 0 ? ((simState.stats.output * simState.stations[0]?.avgTime) / (simState.stats.timeElapsed * simState.stations.length) * 100).toFixed(1) : 0}%</div>
                </div>
            </div>
                ...
                {simState.stations.map((station, idx) => (
                    <React.Fragment key={station.id}>
                        ...
                        <div style={{
                            ...
                            {station.status === 'BUSY' && (
                                <div style={{ width: '60px', height: '4px', backgroundColor: '#333', borderRadius: '2px', marginTop: '10px' }}>
                                    <div style={{
            ...
            <div style={{ display: 'flex', gap: '20px', marginTop: '20px', borderTop: '1px solid #333', paddingTop: '10px', fontSize: '0.8rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><div style={{ width: '10px', height: '10px', backgroundColor: '#0a5', borderRadius: '50%' }} /> Busy</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><div style={{ width: '10px', height: '10px', backgroundColor: '#ff9800', borderRadius: '50%' }} /> Starved (Waiting for Input)</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><div style={{ width: '10px', height: '10px', backgroundColor: '#c50f1f', borderRadius: '50%' }} /> Blocked (Output Full)</div>
            </div>
        </div>
    );
};
