...
import { testSupabaseConnection, getSupabase } from '../utils/supabaseClient';
...
const SystemDiagnostics = () => {
    ...
    const runDiagnostics = async () => {
        ...
        try {
            ...
            addResult('Browser', 'Session Storage', session ? 'success' : 'error', session ? 'Active' : 'Missing');
            setProgress(20);
        } catch (e) {
        ...
        try {
            ...
            addResult('Database', 'IndexedDB', 'success', `${projects.length} Projects`, Math.round(performance.now() - start) + 'ms');
            setProgress(40);
        } catch (e) {
            addResult('Database', 'IndexedDB', 'error', 'Connection Failed: ' + e.message);
        }

        // 3. Cloud Services (Supabase)
        try {
            await sleep(300);
            const start = performance.now();
            const conn = await testSupabaseConnection();
            if (conn.success) {
                addResult('Cloud', 'Supabase API', 'success', 'Connected', Math.round(performance.now() - start) + 'ms');
            } else {
                addResult('Cloud', 'Supabase API', 'error', 'Connection Failed: ' + (conn.error || 'Unknown error'));
            }
            ...
            if (session) {
                addResult('Cloud', 'User Session', 'success', `Logged in as ${session.user.email}`);
            } else {
                addResult('Cloud', 'User Session', 'warning', 'Not Logged In');
            }
            setProgress(60);
        } catch (e) {
            addResult('Cloud', 'Supabase', 'error', 'Check Failed: ' + e.message);
        }
